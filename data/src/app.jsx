...
const DRIVE_LOCAL_ONLY_KEYS = new Set(['vocab_sentences', 'vocab_ui_v1']);
const DRIVE_META_KEY = 'vocab_drive_sync_v1';
const DRIVE_PENDING_KEY = 'vocab_drive_pending_v1';
const DRIVE_SCOPE = 'https://www.googleapis.com/auth/drive.appdata';
const DRIVE_AUTH_TIMEOUT_MS = 90000;
const DRIVE_FETCH_TIMEOUT_MS = 25000;
const DRIVE_OPERATION_TIMEOUT_MS = 120000;
const DRIVE_SNAPSHOT_KIND = 'englishProgressSnapshotV1';
const DRIVE_SNAPSHOT_PREFIX = 'english-progress-v1-';
const DRIVE_SNAPSHOT_SCHEMA = 2;
...
class Component extends DCLogic {
  constructor(p) {
    ...
    this.state = {
      ...
      driveStatus: 'checking', driveMessage: 'در حال بررسی همگام‌سازی…', driveConflictOpen: false,
      driveLastSync: '', driveAccount: '', installHelpOpen: false, installAvailable: false
    };
    this.sentCache = {};
  ...
  // ---- local backup + Google Drive sync ----
  // Drive configuration stays public: a Web OAuth client ID is an identifier,
  // never a client secret. The two values live in <meta> tags so the shipped
  // single HTML can be configured without changing application logic.
  driveConfig(name) {
  ...
  driveEnvironment() {
    ...
    if (protocol === 'https:' || (protocol === 'http:' && localhost)) return 'ready';
    if (protocol === 'file:') return 'file';
    return 'insecure';
  }
  driveMetaLoad() {
    try { return JSON.parse(localStorage.getItem(DRIVE_META_KEY) || '{}') || {}; } catch (e) { return {}; }
  }
  driveMetaSave(patch) {
    const next = Object.assign({}, this.driveMetaLoad(), patch || {});
    try { localStorage.setItem(DRIVE_META_KEY, JSON.stringify(next)); } catch (e) {}
    return next;
  }
  driveRandomId() {
  ...
  driveDeviceId() {
    const meta = this.driveMetaLoad();
    if (meta.deviceId) return meta.deviceId;
    return this.driveMetaSave({ deviceId: this.driveRandomId() }).deviceId;
  }
  driveDeviceLabel() {
  ...
  async driveHash(text) {
    ...
    for (let i = 0; i < src.length; i++) {
      ...
      h2 = Math.imul(h2 ^ src.charCodeAt(i), 3266489917);
    }
    return 'fallback-' + src.length.toString(36) + '-' + (h1 >>> 0).toString(36) + '-' + (h2 >>> 0).toString(36);
  }
  driveProgressDump(dump) {
    const progress = {};
    BACKUP_KEYS.forEach(k => { if (!DRIVE_LOCAL_ONLY_KEYS.has(k)) progress[k] = dump[k]; });
    return progress;
  }
  driveProgressText(dump) { return JSON.stringify(this.driveProgressDump(dump)); }
  driveProgressHash(dump) { return this.driveHash(this.driveProgressText(dump)); }
  driveSummary(dump) {
    const P = (k, d) => { try { return dump[k] ? JSON.parse(dump[k]) : d; } catch (e) { return d; } };
    const core = P('vocab_app_v1', {}), sr = P('vocab_sr_v1', {}), custom = P('vocab_custom', []);
    let introduced = 0, known = 0;
    Object.keys(sr || {}).forEach(k => {
      const rec = sr[k];
      if (!Array.isArray(rec)) return;
      if (rec[4]) introduced++;
      if (rec[8] === 1 || (rec[0] >= 3 && (rec[3] & 6) !== 0 && (rec[2] - rec[1]) >= 7)) known++;
    });
    const activity = ['vocab_sent', 'vocab_course', 'vocab_listen', 'vocab_disc'].reduce((n, k) => {
      const x = P(k, {}); return n + (x && typeof x === 'object' ? Object.keys(x).length : 0);
    }, 0);
    return { seen: Number(core.seen) || 0, introduced: introduced, known: known,
      custom: Array.isArray(custom) ? custom.length : 0, activity: activity };
  }
  driveSummaryText(sum) {
    sum = sum || {};
    return (sum.introduced || 0) + ' واژه آشناشده · ' + (sum.known || 0) + ' بلد · ' + (sum.custom || 0) + ' واژهٔ شخصی';
  }
  driveMeaningful(dump) {
    const s = this.driveSummary(dump);
    if (s.seen || s.introduced || s.known || s.custom || s.activity) return true;
    return ['vocab_overrides', 'vocab_mysent', 'vocab_mycats', 'vocab_catover', 'vocab_famap', 'vocab_game'].some(k => {
      try { const x = dump[k] ? JSON.parse(dump[k]) : null; return !!x && Object.keys(x).length > 0; } catch (e) { return false; }
    });
  }
  driveFormatTime(value) {
    if (!value) return '';
    try { return new Intl.DateTimeFormat('fa-IR', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(value)); }
    catch (e) { try { return new Date(value).toLocaleString(); } catch (e2) { return ''; } }
  }
  driveSetStatus(status, message) {
    if (!this._mounted) return;
    const meta = this.driveMetaLoad();
    this.setState({ driveStatus: status, driveMessage: message || '',
      driveLastSync: meta.lastSyncAt ? this.driveFormatTime(meta.lastSyncAt) : '',
      driveAccount: (this._driveAccount && this._driveAccount.emailAddress) || '' });
  }
  driveClientId() { return this.driveConfig('vocab-google-client-id'); }
  driveOnlineUrl() { return this.driveConfig('vocab-online-url'); }
  driveClientIdValid(value) {
    value = String(value || '').trim();
    return value.indexOf('YOUR_') < 0 && /^[a-z0-9_-]+\.apps\.googleusercontent\.com$/i.test(value);
  }
  driveCurrentOrigin() {
    try { return location.origin && location.origin !== 'null' ? location.origin : ''; } catch (e) { return ''; }
  }
  driveTokenValid() { return !!this._driveToken && Date.now() < (this._driveTokenExpiresAt || 0); }
  async driveBoot() {
    this.driveDeviceId();
    this._driveOnOnline = () => {
      if (this.driveTokenValid()) this.driveSynchronize();
      else if (this.driveMetaLoad().enabled) this.driveSetStatus('reauth', 'اینترنت برگشت؛ برای اتصال دوباره به Drive کلیک کن.');
      else this.driveSetStatus('disconnected', 'پیشرفت فعلاً فقط روی این دستگاه نگه داشته می‌شود.');
    };
    this._driveOnOffline = () => this.driveSetStatus('offline', 'اینترنت قطع است؛ پیشرفت روی این دستگاه ذخیره می‌شود و بعداً همگام خواهد شد.');
    this._driveOnVisible = () => {
      if (document.visibilityState === 'visible' && this.driveTokenValid() && !this._driveConflict) this.driveSynchronize();
    };
    this._driveOnInstall = e => {
      e.preventDefault(); this._driveInstallPrompt = e;
      if (this._mounted) this.setState({ installAvailable: true });
    };
    window.addEventListener('online', this._driveOnOnline);
    window.addEventListener('offline', this._driveOnOffline);
    document.addEventListener('visibilitychange', this._driveOnVisible);
    window.addEventListener('beforeinstallprompt', this._driveOnInstall);
    this._driveWatchIv = setInterval(() => this.driveObserveNow(false), 4000);

    const env = this.driveEnvironment(), clientId = this.driveClientId(), meta = this.driveMetaLoad();
    if (env === 'file') {
      this.driveSetStatus('unsupported', this.driveOnlineUrl()
        ? 'برای انتقال پیشرفت، فایل پشتیبان بگیر و آن را یک‌بار در نسخهٔ آنلاین بازیابی کن.'
        : 'برای Google Drive نسخهٔ آنلاین HTTPS لازم است؛ فایل محلی همچنان آفلاین کار می‌کند.');
    } else if (env !== 'ready') {
      this.driveSetStatus('unsupported', 'همگام‌سازی Drive فقط روی آدرس امن HTTPS فعال می‌شود.');
    } else if (!this.driveClientIdValid(clientId)) {
      this.driveSetStatus('unconfigured', 'دکمه آماده است؛ سازنده باید شناسهٔ Google OAuth را در فایل تنظیم کند.');
    } else if (!navigator.onLine) {
      this.driveSetStatus('offline', 'اینترنت قطع است؛ پیشرفت روی این دستگاه ذخیره می‌شود.');
    } else if (meta.enabled) {
      this.driveSetStatus('reauth', 'برای بررسی آخرین بکاپ، دوباره به Google Drive وصل شو.');
      this.drivePrepareAuth().catch(() => {});
    } else {
      this.driveSetStatus('disconnected', 'پیشرفت فعلاً فقط روی این دستگاه نگه داشته می‌شود.');
      this.drivePrepareAuth().catch(() => {});
    }
    await this.driveObserveNow(false);
  }
  async driveObserveNow(forceDirty) {
    if (this._driveObserving || this._driveApplying) return;
    this._driveObserving = true;
    try {
      const dump = this.collectBackup(), hash = await this.driveProgressHash(dump), meta = this.driveMetaLoad();
      const changed = !!this._driveObservedHash && this._driveObservedHash !== hash;
      this._driveObservedHash = hash;
      if (forceDirty || changed || (meta.baseHash && meta.baseHash !== hash)) {
        this.driveMetaSave({ dirty: true, dirtyAt: new Date().toISOString(), localHash: hash });
        if (this._driveConflict) {
          this.driveSetStatus('conflict', 'هنگام تصمیم‌گیری، پیشرفت تازه هم روی این دستگاه نگه داشته شد.');
        } else if (!navigator.onLine) {
          this.driveSetStatus('offline', 'روی این دستگاه ذخیره شد؛ اینترنت برای بکاپ Drive در دسترس نیست.');
        } else if (this.driveTokenValid()) {
          this.driveSetStatus('pending', 'روی این دستگاه ذخیره شد؛ همگام‌سازی Drive در صف است.');
          this.driveQueueSync(30000);
        } else if (meta.enabled) {
          this.driveSetStatus('reauth', 'روی این دستگاه ذخیره شد؛ برای بکاپ Drive دوباره کلیک کن.');
        }
      }
    } finally { this._driveObserving = false; }
  }
  driveQueueSync(delay) {
    clearTimeout(this._driveSyncTimer);
    this._driveSyncTimer = setTimeout(() => {
      if (this.driveTokenValid() && navigator.onLine && !this._driveConflict) this.driveSynchronize();
    }, delay == null ? 30000 : delay);
  }
  drivePrepareAuth() {
    if (this._driveGisPromise) return this._driveGisPromise;
    this._driveGisPromise = new Promise((resolve, reject) => {
      let settled = false, poll = null, timeout = null;
      const finish = (fn, value) => {
        if (settled) return; settled = true;
        clearInterval(poll); clearTimeout(timeout); fn(value);
      };
      const ready = () => {
        try {
          if (!window.google || !google.accounts || !google.accounts.oauth2) throw new Error('کتابخانهٔ ورود گوگل آماده نشد.');
          this._driveAuthReady = true;
          finish(resolve, true);
        } catch (e) { finish(reject, e); }
      };
      if (window.google && google.accounts && google.accounts.oauth2) return ready();
      let script = document.getElementById('vocab-google-gis');
      let created = false;
      if (!script) {
        script = document.createElement('script'); script.id = 'vocab-google-gis';
      ...
      script.addEventListener('error', () => {
        ...
        finish(reject, new Error('کتابخانهٔ ورود گوگل بارگذاری نشد.'));
      }, { once: true });
      poll = setInterval(() => {
      ...
      timeout = setTimeout(() => {
        try { script.remove(); } catch (e) {}
        finish(reject, new Error('بارگذاری اتصال گوگل طول کشید؛ دوباره تلاش کن.'));
      }, 10000);
      if (created) document.head.appendChild(script);
    }).catch(err => { this._driveGisPromise = null; throw err; });
    return this._driveGisPromise;
  }
  driveClearAuthTimeout() {
    clearTimeout(this._driveAuthTimer); this._driveAuthTimer = null;
  }
  driveArmAuthTimeout(generation) {
    this.driveClearAuthTimeout();
    if (this._driveAuthPending !== generation || this._driveAuthGeneration !== generation) return;
    this._driveAuthTimer = setTimeout(() => {
      if (this._driveAuthPending !== generation || this._driveAuthGeneration !== generation) return;
      this._driveAuthTimer = null;
      this._driveAuthPending = 0;
      this._driveAuthGeneration = generation + 1;
      this._driveTokenClient = null;
      const origin = this.driveCurrentOrigin();
      this.driveSetStatus('error', 'گوگل پاسخی نداد؛ پنجره‌های بازشو را مجاز کن و دوباره تلاش کن.' +
        (origin ? ' اگر تکرار شد، نشانی مجاز OAuth باید دقیقاً «' + origin + '» باشد.' : ''));
    }, DRIVE_AUTH_TIMEOUT_MS);
  }
  driveCancelAuthAttempt(message) {
    if (!this._driveAuthPending) return false;
    this._driveAuthGeneration = (this._driveAuthGeneration || 0) + 1;
    this._driveAuthPending = 0;
    this._driveTokenClient = null;
    this.driveClearAuthTimeout();
    if (message) this.driveSetStatus('reauth', message);
    return true;
  }
  driveOAuthErrorMessage(response) {
    const code = String((response && response.error) || '').toLowerCase();
    const origin = this.driveCurrentOrigin();
    if (code === 'origin_mismatch' || code === 'redirect_uri_mismatch' || code === 'invalid_client') {
      return 'تنظیم OAuth با آدرس این سایت هماهنگ نیست.' +
        (origin ? ' در Google Cloud، Authorized JavaScript origin را دقیقاً «' + origin + '» بگذار.' : '');
    }
    if (code === 'access_denied') return 'مجوز Google Drive داده نشد؛ برای تلاش دوباره کلیک کن.';
    return 'اتصال Google Drive کامل نشد؛ تنظیم Client ID و آدرس مجاز سایت را بررسی و دوباره تلاش کن.';
  }
  async driveConnect() {
    const env = this.driveEnvironment(), clientId = this.driveClientId();
    if (env !== 'ready') {
      const url = this.driveOnlineUrl();
      if (env === 'file' && url) {
        window.open(url, '_blank', 'noopener');
        this.exportBackup();
        return this.driveSetStatus('unsupported', 'فایل پشتیبان دانلود شد؛ در نسخهٔ آنلاین از تنظیمات آن را بازیابی کن.');
      }
      if (url) { window.open(url, '_blank', 'noopener'); return; }
      return this.driveSetStatus('unsupported', env === 'file' ? 'نسخهٔ آنلاین لازم است؛ آدرس آن هنوز در فایل تنظیم نشده.' : 'این صفحه باید از HTTPS باز شود.');
    }
    if (!this.driveClientIdValid(clientId)) return this.driveSetStatus('unconfigured', 'شناسهٔ Google OAuth تنظیم نشده یا معتبر نیست.');
    if (!navigator.onLine) return this.driveSetStatus('offline', 'اینترنت قطع است؛ پیشرفت روی این دستگاه محفوظ می‌ماند.');
    if (this.driveTokenValid()) {
      if (this._driveConflict) {
        if (this._mounted) this.setState({ driveConflictOpen: true });
        return;
      }
      return this.driveSynchronize();
    }
    if (this._driveAuthPending) {
      this.driveCancelAuthAttempt('تلاش قبلی لغو شد؛ برای باز کردن دوبارهٔ پنجرهٔ گوگل کلیک کن.');
      return;
    }
    if (!this._driveAuthReady) {
      this.driveSetStatus('connecting', 'در حال آماده‌کردن اتصال گوگل…');
      const prepareGeneration = this._driveAuthGeneration || 0;
      this.drivePrepareAuth().then(() => {
        if (!this._mounted || prepareGeneration !== (this._driveAuthGeneration || 0) || this.driveTokenValid()) return;
        this.driveSetStatus('reauth', 'اتصال آماده شد؛ برای انتخاب حساب یک بار دیگر کلیک کن.');
      }).catch(err => {
        if (!this._mounted || prepareGeneration !== (this._driveAuthGeneration || 0)) return;
        this.driveSetStatus('error', err && err.message ? err.message : 'اتصال به گوگل آماده نشد.');
      });
      return;
    }
    const generation = (this._driveAuthGeneration || 0) + 1;
    this._driveAuthGeneration = generation;
    this._driveAuthPending = generation;
    this.driveSetStatus('connecting', 'در حال باز کردن انتخاب حساب گوگل…');
    try {
      const client = google.accounts.oauth2.initTokenClient({
        client_id: clientId, scope: DRIVE_SCOPE,
        callback: response => this.driveTokenResponse(response, generation),
        error_callback: err => this.driveTokenError(err, generation)
      });
      this._driveTokenClient = client;
      client.requestAccessToken({ prompt: 'select_account' });
      this.driveArmAuthTimeout(generation);
    } catch (err) {
      if (this._driveAuthPending !== generation || this._driveAuthGeneration !== generation) return;
      this._driveAuthPending = 0;
      this.driveClearAuthTimeout();
      this.driveSetStatus('error', err && err.message ? err.message : 'اتصال به گوگل ممکن نشد؛ دوباره تلاش کن.');
    }
  }
  driveTokenError(err, generation) {
    if (this._driveAuthPending !== generation || this._driveAuthGeneration !== generation) return;
    this._driveAuthPending = 0;
    this.driveClearAuthTimeout();
    const type = err && err.type;
    const msg = type === 'popup_closed' ? 'پنجرهٔ اتصال بسته شد؛ هر وقت آماده بودی دوباره بزن.' :
      type === 'popup_failed_to_open' ? 'مرورگر پنجرهٔ گوگل را مسدود کرد؛ Pop-up این سایت را مجاز کن و دوباره بزن.' :
      'پنجرهٔ گوگل پاسخ نداد؛ دوباره تلاش کن.';
    this.driveSetStatus('reauth', msg);
  }
  driveTokenResponse(response, generation) {
    if (this._driveAuthPending !== generation || this._driveAuthGeneration !== generation) {
      try { if (response && response.access_token && response.access_token !== this._driveToken && window.google && google.accounts.oauth2) google.accounts.oauth2.revoke(response.access_token, () => {}); } catch (e) {}
      return;
    }
    this._driveAuthPending = 0;
    this.driveClearAuthTimeout();
    if (!response || response.error || !response.access_token) return this.driveSetStatus('reauth', this.driveOAuthErrorMessage(response));
    try {
      if (!google.accounts.oauth2.hasGrantedAllScopes(response, DRIVE_SCOPE)) {
        try { google.accounts.oauth2.revoke(response.access_token, () => {}); } catch (e) {}
        return this.driveSetStatus('reauth', 'مجوز لازم برای بکاپ Drive کامل داده نشد.');
      }
    } catch (e) {}
    // A renewed token may belong to a different Google account. Rebuild any
    // in-memory conflict only after about.get identifies the new account.
    this._driveConflict = null; this._driveAccount = null;
    if (this._mounted) this.setState({ driveConflictOpen: false, driveAccount: '' });
    this._driveToken = response.access_token;
    this._driveTokenExpiresAt = Date.now() + Math.max(0, (Number(response.expires_in) || 3600) * 1000 - 60000);
    clearTimeout(this._driveTokenTimer);
    this._driveTokenTimer = setTimeout(() => {
      this._driveToken = null;
      this.driveCancelOperations(); this._driveSyncing = false; this._driveResolving = false;
      if (this._mounted) this.setState({ driveConflictOpen: false });
      this.driveSetStatus('reauth', 'اتصال Drive تمام شد؛ برای ادامهٔ بکاپ دوباره کلیک کن.');
    }, Math.max(1000, this._driveTokenExpiresAt - Date.now()));
    this.driveMetaSave({ enabled: true });
    this.driveSynchronize();
  }
  driveDisconnect() {
    ...
    this.driveClearAuthTimeout();
    this.driveCancelOperations(); this._driveSyncing = false; this._driveResolving = false;
    this._driveToken = null; this._driveTokenExpiresAt = 0; clearTimeout(this._driveTokenTimer);
    this._driveConflict = null; this._driveAccount = null;
    this.driveMetaSave({ enabled: false });
    try { if (token && window.google && google.accounts.oauth2) google.accounts.oauth2.revoke(token, () => {}); } catch (e) {}
    if (this._mounted) this.setState({ driveConflictOpen: false, driveAccount: '' });
    this.driveSetStatus('disconnected', 'اتصال Drive قطع شد؛ پیشرفت فقط روی این دستگاه نگه داشته می‌شود.');
  }
  driveBeginOperation() {
    this.driveCancelOperations();
    const opId = this._driveOpId;
    try { this._driveAbortController = new AbortController(); } catch (e) { this._driveAbortController = null; }
    return opId;
  }
  driveClearOperationTimeout() {
    clearTimeout(this._driveOperationTimer); this._driveOperationTimer = null;
  }
  driveArmOperationTimeout(opId) {
    this.driveClearOperationTimeout();
    this._driveOperationTimer = setTimeout(() => {
      if (opId !== this._driveOpId) return;
      this.driveCancelOperations();
      this._driveSyncing = false; this._driveResolving = false;
      this.driveSetStatus('error', 'همگام‌سازی بیش از حد طول کشید و متوقف شد؛ پیشرفت روی این دستگاه محفوظ است. دوباره تلاش کن.');
    }, DRIVE_OPERATION_TIMEOUT_MS);
  }
  driveCancelOperations() {
    this.driveClearOperationTimeout();
    this._driveOpId = (this._driveOpId || 0) + 1;
    try { if (this._driveAbortController) this._driveAbortController.abort(); } catch (e) {}
    this._driveAbortController = null;
    return this._driveOpId;
  }
  driveGuard(opId) {
    if (opId != null && opId !== this._driveOpId) {
      const err = new Error('عملیات همگام‌سازی متوقف شد.'); err.code = 'CANCELLED'; throw err;
    }
  }
  async driveFetch(url, options, opId) {
    this.driveGuard(opId);
    if (!this.driveTokenValid()) {
      const err = new Error('اتصال دوباره به Google Drive لازم است.'); err.code = 'REAUTH'; throw err;
    }
    options = Object.assign({}, options || {});
    const headers = new Headers(options.headers || {});
    headers.set('Authorization', 'Bearer ' + this._driveToken); options.headers = headers;
    const suppliedSignal = options.signal || null;
    delete options.signal;
    const operationSignal = opId != null && opId === this._driveOpId && this._driveAbortController ? this._driveAbortController.signal : null;
    let last;
    for (let attempt = 0; attempt < 3; attempt++) {
      let response, timedOut = false, timer = null, controller = null;
      const linked = [];
      const attemptOptions = Object.assign({}, options);
      try {
        if (typeof AbortController !== 'undefined') {
          controller = new AbortController(); attemptOptions.signal = controller.signal;
          const link = signal => {
            if (!signal) return;
            if (signal.aborted) return controller.abort();
            const relay = () => controller.abort();
            signal.addEventListener('abort', relay, { once: true }); linked.push([signal, relay]);
          };
          link(suppliedSignal); link(operationSignal);
          timer = setTimeout(() => { timedOut = true; controller.abort(); }, DRIVE_FETCH_TIMEOUT_MS);
        } else if (operationSignal || suppliedSignal) attemptOptions.signal = operationSignal || suppliedSignal;
        response = await fetch(url, attemptOptions); this.driveGuard(opId);
        // Buffer the body while the attempt timeout and operation abort are
        // still active. fetch() itself resolves after headers, while json()/
        // text() could otherwise leave the UI waiting forever on a stalled body.
        const method = String(attemptOptions.method || 'GET').toUpperCase();
        const hasNoBody = method === 'HEAD' || response.status === 204 || response.status === 205;
        ...
        if (((e && e.name === 'AbortError') && !timedOut) || (opId != null && opId !== this._driveOpId)) {
          const stopped = new Error('عملیات همگام‌سازی متوقف شد.'); stopped.code = 'CANCELLED'; throw stopped;
        }
        last = e;
        if (attempt < 2 && navigator.onLine) { await new Promise(r => setTimeout(r, 500 * Math.pow(2, attempt) + Math.random() * 250)); this.driveGuard(opId); continue; }
        const err = new Error(timedOut ? 'Google Drive در زمان مناسب پاسخ نداد.' : 'ارتباط اینترنت یا Google Drive در دسترس نیست.'); err.code = 'NETWORK'; throw err;
      } finally {
        clearTimeout(timer);
        linked.forEach(pair => { try { pair[0].removeEventListener('abort', pair[1]); } catch (e) {} });
      }
      if (response.status === 401) {
        this._driveToken = null; this._driveTokenExpiresAt = 0;
        const err = new Error('اتصال دوباره به Google Drive لازم است.'); err.code = 'REAUTH'; throw err;
      }
      if (!response.ok) {
        let details = null;
        try { details = JSON.parse(await response.text()); } catch (e) {}
        this.driveGuard(opId);
        const reason = details && details.error && details.error.errors && details.error.errors[0] && details.error.errors[0].reason;
        const rateLimited = response.status === 403 && (reason === 'rateLimitExceeded' || reason === 'userRateLimitExceeded');
        if ((response.status === 429 || response.status >= 500 || rateLimited) && attempt < 2) {
          await new Promise(r => setTimeout(r, 500 * Math.pow(2, attempt) + Math.random() * 250));
          this.driveGuard(opId); continue;
        }
        const err = new Error(details && details.error && details.error.message ? details.error.message : 'خطای Google Drive (' + response.status + ')');
        err.status = response.status; err.reason = reason; err.details = details; throw err;
      }
      return response;
    }
    throw last || new Error('Google Drive پاسخ نداد.');
  }
  async driveLoadAccount(opId) {
    const response = await this.driveFetch('https://www.googleapis.com/drive/v3/about?fields=user(permissionId,displayName,emailAddress)', null, opId);
    const about = await response.json(), account = about.user || {};
    this.driveGuard(opId);
    if (!account.permissionId) throw new Error('شناسهٔ حساب Google Drive دریافت نشد؛ همگام‌سازی متوقف شد.');
    this._driveAccount = account;
    const meta = this.driveMetaLoad();
    if (meta.accountId && account.permissionId && meta.accountId !== account.permissionId) {
      try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
      this.driveMetaSave({ accountId: account.permissionId, baseSnapshotId: '', baseHash: '',
        localHash: '', lastSyncAt: '', dirty: true });
    } else if (account.permissionId) this.driveMetaSave({ accountId: account.permissionId });
    if (this._mounted) this.setState({ driveAccount: account.emailAddress || '' });
    return account;
  }
  async installShortcut() {
    if (this._driveInstallPrompt) {
      const prompt = this._driveInstallPrompt; this._driveInstallPrompt = null;
  ...
  driveParents(file) {
    const validParents = values => Array.isArray(values) && values.length <= 90 &&
      values.every(value => typeof value === 'string' && value.length > 0 && value.length <= 200) &&
      new Set(values).size === values.length;
    const description = String((file && file.description) || '');
    if (description.indexOf('vocab-parents-v1:') === 0) {
      try {
        const parsed = JSON.parse(description.slice('vocab-parents-v1:'.length));
        if (validParents(parsed)) return parsed.slice();
      } catch (e) {}
    }
    const props = (file && file.appProperties) || {};
    const legacy = Object.keys(props).filter(k => /^p\d+$/.test(k)).sort((a, b) => Number(a.slice(1)) - Number(b.slice(1))).map(k => props[k]).filter(Boolean);
    return validParents(legacy) ? legacy : [];
  }
  driveHeads(files) {
    const referenced = new Set();
    (files || []).forEach(f => this.driveParents(f).forEach(id => referenced.add(id)));
    return (files || []).filter(f => !referenced.has(f.id));
  }
  async driveListSnapshots(opId) {
    const all = []; let pageToken = '';
    do {
      const params = new URLSearchParams({
        spaces: 'appDataFolder', pageSize: '1000', orderBy: 'createdTime',
        q: "'appDataFolder' in parents and trashed = false and appProperties has { key='vocabKind' and value='" + DRIVE_SNAPSHOT_KIND + "' }",
        fields: 'nextPageToken,files(id,name,description,createdTime,modifiedTime,size,appProperties)'
      });
      if (pageToken) params.set('pageToken', pageToken);
      const response = await this.driveFetch('https://www.googleapis.com/drive/v3/files?' + params.toString(), null, opId);
      const result = await response.json();
      this.driveGuard(opId);
      (result.files || []).forEach(f => all.push(f));
      pageToken = result.nextPageToken || '';
      if (all.length > 10000) throw new Error('تعداد نسخه‌های Drive بیش از حد انتظار است؛ همگام‌سازی متوقف شد.');
    } while (pageToken);
    return all;
  }
  async driveDownloadSnapshot(file, opId) {
    const response = await this.driveFetch('https://www.googleapis.com/drive/v3/files/' + encodeURIComponent(file.id) + '?alt=media', null, opId);
    const text = await response.text();
    this.driveGuard(opId);
    if (text.length > 10 * 1024 * 1024) throw new Error('یکی از نسخه‌های Drive بیش از حد بزرگ است.');
    let envelope;
    try { envelope = JSON.parse(text); } catch (e) { throw new Error('یکی از نسخه‌های Drive خراب است و بازنویسی نشد.'); }
    if (!envelope || envelope.kind !== DRIVE_SNAPSHOT_KIND || (envelope.schema !== 1 && envelope.schema !== DRIVE_SNAPSHOT_SCHEMA) || envelope.snapshotId !== file.id) {
      throw new Error('یکی از نسخه‌های Drive ساختار ناشناخته‌ای دارد.');
    }
    const storedDump = envelope.schema === 1 ? this.validateBackup(envelope.payload, false) : this.validateBackup(envelope.payload, true);
    const storedHash = await this.driveHash(JSON.stringify(storedDump));
    this.driveGuard(opId);
    if (storedHash !== envelope.payloadHash || ((file.appProperties || {}).hash && (file.appProperties || {}).hash !== storedHash)) {
      throw new Error('صحت یکی از نسخه‌های Drive تأیید نشد؛ هیچ داده‌ای جایگزین نشد.');
    }
    const dump = envelope.schema === 1 ? this.normalizeLegacyBackup(storedDump) : storedDump;
    const hash = await this.driveProgressHash(dump);
    this.driveGuard(opId);
    if (envelope.schema === DRIVE_SNAPSHOT_SCHEMA && envelope.progressHash !== hash) {
      throw new Error('صحت پیشرفت یکی از نسخه‌های Drive تأیید نشد.');
    }
    if ((file.appProperties || {}).progress && (file.appProperties || {}).progress !== hash) {
      throw new Error('شناسهٔ پیشرفت یکی از نسخه‌های Drive یکسان نیست.');
    }
    if (!Array.isArray(envelope.parents) || envelope.parents.length > 90 ||
        envelope.parents.some(id => typeof id !== 'string' || !id || id.length > 200) ||
        new Set(envelope.parents).size !== envelope.parents.length) throw new Error('زنجیرهٔ یکی از نسخه‌های Drive معتبر نیست.');
    const metaParents = this.driveParents(file), bodyParents = envelope.parents;
    if (metaParents.join('|') !== bodyParents.join('|')) throw new Error('زنجیرهٔ یکی از نسخه‌های Drive معتبر نیست.');
    return { id: file.id, file: file, envelope: envelope, dump: dump, hash: hash, payloadHash: storedHash,
      needsMigration: envelope.schema !== DRIVE_SNAPSHOT_SCHEMA,
      summary: envelope.summary || this.driveSummary(dump), createdAt: envelope.createdAt || file.createdTime || file.modifiedTime };
  }
  driveMultipart(metadata, payload) {
    const boundary = 'vocab_drive_' + this.driveRandomId().replace(/[^a-z0-9]/gi, '');
    const crlf = '\r\n', type = 'multipart/related; boundary=' + boundary;
    const body = new Blob([
      '--' + boundary + crlf, 'Content-Type: application/json; charset=UTF-8' + crlf + crlf,
      JSON.stringify(metadata), crlf,
      '--' + boundary + crlf, 'Content-Type: application/json; charset=UTF-8' + crlf + crlf,
      JSON.stringify(payload), crlf, '--' + boundary + '--' + crlf
    ], { type: type });
    return { body: body, type: type };
  }
  async driveGenerateFileId(opId) {
    const response = await this.driveFetch('https://www.googleapis.com/drive/v3/files/generateIds?count=1&space=appDataFolder&type=files', null, opId);
    const result = await response.json();
    this.driveGuard(opId);
    if (!result.ids || !result.ids[0]) throw new Error('Google Drive شناسهٔ نسخه را نساخت.');
    return result.ids[0];
  }
  driveSnapshotProperties(envelope) {
    return { vocabKind: DRIVE_SNAPSHOT_KIND, schema: String(envelope.schema || DRIVE_SNAPSHOT_SCHEMA), hash: envelope.payloadHash,
      progress: envelope.progressHash, device: String(envelope.deviceId || '').slice(0, 100) };
  }
  driveSnapshotDescription(envelope) {
    return 'vocab-parents-v1:' + JSON.stringify((envelope.parents || []).map(String));
  }
  async driveUploadEnvelope(envelope, opId) {
    const metadata = { id: envelope.snapshotId, name: DRIVE_SNAPSHOT_PREFIX + envelope.snapshotId + '.json',
      mimeType: 'application/json', parents: ['appDataFolder'], description: this.driveSnapshotDescription(envelope),
      appProperties: this.driveSnapshotProperties(envelope) };
    const multi = this.driveMultipart(metadata, envelope);
    try {
      const response = await this.driveFetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,createdTime,modifiedTime,size,appProperties',
        { method: 'POST', headers: { 'Content-Type': multi.type }, body: multi.body }, opId);
      const file = await response.json();
      this.driveGuard(opId);
      return file;
    } catch (err) {
      if (err && err.status === 409) {
        const metaResponse = await this.driveFetch('https://www.googleapis.com/drive/v3/files/' + encodeURIComponent(envelope.snapshotId) +
          '?fields=id,name,description,createdTime,modifiedTime,size,appProperties', null, opId);
        const actualFile = await metaResponse.json();
        this.driveGuard(opId);
        let existing;
        try { existing = await this.driveDownloadSnapshot(actualFile, opId); }
        catch (readErr) {
          if (readErr && (readErr.code === 'NETWORK' || readErr.code === 'REAUTH' || readErr.code === 'CANCELLED' ||
              readErr.status === 403 || readErr.status === 429 || readErr.status >= 500)) throw readErr;
          const clash = new Error('شناسهٔ نسخه در Drive قبلاً با دادهٔ دیگری استفاده شده است.');
          clash.code = 'SNAPSHOT_ID_CONFLICT'; clash.actualFile = actualFile; clash.cause = readErr; throw clash;
        }
        if (existing.payloadHash === envelope.payloadHash && this.driveParents(actualFile).join('|') === (envelope.parents || []).join('|')) return actualFile;
        const clash = new Error('نسخهٔ موجود در Drive با آپلود ناتمام فرق دارد؛ هر دو نسخه حفظ می‌شوند.');
        clash.code = 'SNAPSHOT_ID_CONFLICT'; clash.existing = existing; throw clash;
      }
      throw err;
    }
  }
  driveSavePending(envelope) {
    const accountId = (this._driveAccount && this._driveAccount.permissionId) || this.driveMetaLoad().accountId || '';
    try {
      localStorage.setItem(DRIVE_PENDING_KEY, JSON.stringify({ accountId: accountId, envelope: envelope }));
    } catch (e) {
      // Only snapshots whose payload is the local state may be regenerated.
      // A user's conflict choice or a remote merge must never be substituted
      // with whatever happens to be local after a reload.
      if (!this.drivePendingCanRegenerate(envelope.reason)) throw new Error('فضای مرورگر برای نگهداری امن انتخاب همگام‌سازی کافی نیست؛ هیچ نسخه‌ای آپلود نشد.');
      try {
        localStorage.setItem(DRIVE_PENDING_KEY, JSON.stringify({ accountId: accountId,
          envelope: Object.assign({}, envelope, { payload: null }) }));
      } catch (smallErr) {
        throw new Error('فضای مرورگر برای ثبت امن بکاپ ناتمام کافی نیست؛ هیچ نسخه‌ای آپلود نشد.');
      }
    }
  }
  drivePendingCanRegenerate(reason) {
    return ['first-backup', 'local-change', 'replace-empty-cloud', 'conflicting-local-branch',
      'local-migration', 'retry-after-ambiguous-upload'].indexOf(reason) >= 0;
  }
  async driveCreateSnapshot(dump, hash, parents, reason, opId, context) {
    dump = this.validateBackup(dump, true);
    if ((parents || []).length > 90) throw new Error('شاخه‌های هم‌زمان بیش از حد انتظار است؛ برای ایمنی همگام‌سازی متوقف شد.');
    const payloadHash = await this.driveHash(JSON.stringify(dump));
    const progressHash = await this.driveProgressHash(dump);
    this.driveGuard(opId);
    if (hash && hash !== progressHash) throw new Error('پیشرفت محلی هنگام ساخت نسخه تغییر کرد؛ همگام‌سازی دوباره بررسی می‌شود.');
    const fileId = await this.driveGenerateFileId(opId);
    this.driveGuard(opId);
    const envelope = { kind: DRIVE_SNAPSHOT_KIND, schema: DRIVE_SNAPSHOT_SCHEMA, snapshotId: fileId, parents: (parents || []).slice(),
      deviceId: this.driveDeviceId(), deviceLabel: this.driveDeviceLabel(), createdAt: new Date().toISOString(),
      payloadHash: payloadHash, progressHash: progressHash, summary: this.driveSummary(dump),
      reason: reason || 'sync', payload: dump };
    if (context && context.localHashBefore) envelope.localHashBefore = context.localHashBefore;
    if (context && Object.prototype.hasOwnProperty.call(context, 'advanceBase')) envelope.advanceBase = !!context.advanceBase;
    this.driveSavePending(envelope);
    const file = await this.driveUploadEnvelope(envelope, opId);
    this.driveGuard(opId);
    if (envelope.reason !== 'user-resolution') {
      try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
    }
    return { id: fileId, file: file, envelope: envelope, dump: dump, hash: progressHash, payloadHash: payloadHash,
      summary: envelope.summary, createdAt: envelope.createdAt };
  }
  async driveResumePending(opId) {
    let pending = null;
    try { pending = JSON.parse(localStorage.getItem(DRIVE_PENDING_KEY) || 'null'); } catch (e) {}
    if (!pending || !pending.envelope) return false;
    const accountId = (this._driveAccount && this._driveAccount.permissionId) || '';
    if (!pending.accountId || !accountId || pending.accountId !== accountId) {
      try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
      return false;
    }
    let envelope = pending.envelope;
    if (!envelope.snapshotId || envelope.kind !== DRIVE_SNAPSHOT_KIND) {
      throw new Error('رکورد بکاپ ناتمام معتبر نیست.');
    }
    if (!envelope.payload) {
      if (!this.drivePendingCanRegenerate(envelope.reason)) {
        try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
        return false;
      }
      const dump = this.collectBackup(); envelope.payload = dump;
      envelope.payloadHash = await this.driveHash(JSON.stringify(dump));
      envelope.progressHash = await this.driveProgressHash(dump); envelope.summary = this.driveSummary(dump);
      this.driveGuard(opId);
      envelope.schema = DRIVE_SNAPSHOT_SCHEMA;
      envelope.deviceId = this.driveDeviceId(); envelope.deviceLabel = this.driveDeviceLabel();
      this.driveSavePending(envelope);
    } else if (envelope.schema === 1) {
      const sparse = this.validateBackup(envelope.payload, false);
      const oldHash = await this.driveHash(JSON.stringify(sparse));
      this.driveGuard(opId);
      if (oldHash !== envelope.payloadHash) throw new Error('رکورد بکاپ ناتمام خراب است.');
      envelope.payload = this.normalizeLegacyBackup(sparse);
      envelope.payloadHash = await this.driveHash(JSON.stringify(envelope.payload));
      envelope.progressHash = await this.driveProgressHash(envelope.payload);
      this.driveGuard(opId);
      envelope.schema = DRIVE_SNAPSHOT_SCHEMA; envelope.summary = this.driveSummary(envelope.payload);
      this.driveSavePending(envelope);
    } else if (envelope.schema !== DRIVE_SNAPSHOT_SCHEMA) {
      throw new Error('نسخهٔ رکورد بکاپ ناتمام پشتیبانی نمی‌شود.');
    }
    let validPayload = this.validateBackup(envelope.payload, true);
    const validPayloadHash = await this.driveHash(JSON.stringify(validPayload));
    const validProgressHash = await this.driveProgressHash(validPayload);
    this.driveGuard(opId);
    if (validPayloadHash !== envelope.payloadHash || validProgressHash !== envelope.progressHash) {
      throw new Error('رکورد بکاپ ناتمام با دادهٔ ذخیره‌شده یکسان نیست.');
    }
    try {
      await this.driveUploadEnvelope(envelope, opId);
    } catch (err) {
      if (!err || err.code !== 'SNAPSHOT_ID_CONFLICT') throw err;
      this.driveGuard(opId);
      try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
      const quarantinedId = (err.existing && err.existing.id) || (err.actualFile && err.actualFile.id) || '';
      if (!this.drivePendingCanRegenerate(envelope.reason)) {
        const resolutionParents = Array.from(new Set((envelope.parents || []).concat(quarantinedId ? [quarantinedId] : [])));
        const freshResolution = await this.driveCreateSnapshot(validPayload, envelope.progressHash, resolutionParents,
          envelope.reason, opId, { localHashBefore: envelope.localHashBefore, advanceBase: envelope.advanceBase });
        this.driveGuard(opId);
        envelope = freshResolution.envelope; validPayload = freshResolution.dump;
      } else {
      const currentDump = this.collectBackup(), currentHash = await this.driveProgressHash(currentDump);
      this.driveGuard(opId);
      if (err.existing && err.existing.hash === currentHash) {
        this.driveMetaSave({ baseSnapshotId: err.existing.id, baseHash: currentHash,
          localHash: currentHash, dirty: false });
        return true;
      }
      const parents = quarantinedId ? [quarantinedId] : (envelope.parents || []);
      const fresh = await this.driveCreateSnapshot(currentDump, currentHash, parents, 'retry-after-ambiguous-upload', opId);
      this.driveGuard(opId);
      const latestHash = await this.driveProgressHash(this.collectBackup());
      this.driveGuard(opId);
      this.driveMetaSave({ baseSnapshotId: fresh.id, baseHash: currentHash,
        localHash: latestHash, dirty: latestHash !== currentHash });
      return true;
      }
    }
    this.driveGuard(opId);
    const currentBeforeResume = this.collectBackup(), currentProgressText = this.driveProgressText(currentBeforeResume);
    let currentHash = await this.driveHash(currentProgressText);
    this.driveGuard(opId);
    if (envelope.reason === 'user-resolution' && currentHash !== envelope.progressHash &&
        envelope.localHashBefore && currentHash === envelope.localHashBefore) {
      let applied;
      try {
        applied = await this.driveApplySnapshot({ id: envelope.snapshotId, hash: envelope.progressHash,
          dump: validPayload, envelope: envelope }, opId, currentProgressText);
      } catch (err) {
        if (!err || err.code !== 'LOCAL_CHANGED') throw err;
        const newestHash = await this.driveProgressHash(this.collectBackup());
        this.driveGuard(opId);
        this.driveMetaSave({ localHash: newestHash, dirty: true, dirtyAt: new Date().toISOString() });
        try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
        return true;
      }
      currentHash = applied.hash;
      this.driveGuard(opId);
    }
    const mayAdvanceBase = envelope.reason !== 'automatic-same-data' || currentHash === envelope.progressHash || envelope.advanceBase === true;
    if ((envelope.reason !== 'user-resolution' || currentHash === envelope.progressHash) && mayAdvanceBase) {
      this.driveMetaSave({ baseSnapshotId: envelope.snapshotId, baseHash: envelope.progressHash,
        localHash: currentHash, dirty: currentHash !== envelope.progressHash });
    } else {
      this.driveMetaSave({ localHash: currentHash, dirty: true, dirtyAt: new Date().toISOString() });
    }
    try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
    return true;
  }
  driveMarkSynced(snapshot, localHash) {
    const now = new Date().toISOString();
    this._driveObservedHash = localHash;
    this.driveMetaSave({ baseSnapshotId: snapshot.id, baseHash: localHash, localHash: localHash,
      dirty: false, lastSyncAt: now, enabled: true });
    this._driveConflict = null;
    if (this._mounted) this.setState({ driveConflictOpen: false });
    this.driveSetStatus('synced', 'پیشرفت این دستگاه با Google Drive همگام است.');
  }
  async driveApplySnapshot(snapshot, opId, expectedProgressText) {
    this.driveGuard(opId);
    const localBefore = this.collectBackup(), merged = Object.assign({}, snapshot.dump);
    if (expectedProgressText != null && this.driveProgressText(localBefore) !== expectedProgressText) {
      const err = new Error('پیشرفت این دستگاه هنگام بازیابی تغییر کرد.'); err.code = 'LOCAL_CHANGED'; throw err;
    }
    DRIVE_LOCAL_ONLY_KEYS.forEach(k => { merged[k] = localBefore[k]; });
    this._driveApplying = true;
    try { this.applyBackup(merged); } finally { this._driveApplying = false; }
    const current = this.collectBackup(), hash = await this.driveProgressHash(current);
    this.driveGuard(opId);
    this._driveObservedHash = hash;
    this.driveMetaSave({ baseSnapshotId: snapshot.id, baseHash: snapshot.hash, localHash: hash,
      dirty: hash !== snapshot.hash, enabled: true });
    return { dump: current, hash: hash };
  }
  driveShowConflict(heads, localHash) {
    const unique = [], seen = new Set();
    heads.slice().sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt))).forEach(h => {
      if (seen.has(h.hash)) return; seen.add(h.hash);
      unique.push({ id: h.id, hash: h.hash, dump: h.dump, summary: h.summary, createdAt: h.createdAt,
        isLocal: h.hash === localHash, deviceLabel: h.envelope.deviceLabel || 'دستگاه دیگر' });
    });
    this._driveConflict = { heads: heads, headIds: heads.map(h => h.id).sort(), choices: unique, localHash: localHash };
    this.driveSetStatus('conflict', 'دو پیشرفت متفاوت پیدا شد؛ انتخاب کن کدام نسخه ادامه پیدا کند.');
    if (this._mounted) this.setState({ driveConflictOpen: true });
  }
  async driveSyncCore(depth, opId) {
    if (depth > 8) throw new Error('همگام‌سازی چند بار تغییر کرد؛ لطفاً دوباره دکمه را بزن.');
    const dump = this.collectBackup(), localProgressText = this.driveProgressText(dump);
    const localHash = await this.driveHash(localProgressText), meta = this.driveMetaLoad();
    this.driveGuard(opId);
    const files = await this.driveListSnapshots(opId), fileById = {};
    this.driveGuard(opId);
    files.forEach(f => { fileById[f.id] = f; });
    const headFiles = this.driveHeads(files);
    if (!headFiles.length) {
      const created = await this.driveCreateSnapshot(dump, localHash, [], 'first-backup', opId);
      this.driveGuard(opId);
      this.driveMetaSave({ baseSnapshotId: created.id, baseHash: localHash });
      return this.driveSyncCore(depth + 1, opId);
    }
    if (headFiles.length > 30) throw new Error('تعداد شاخه‌های هم‌زمان غیرعادی است؛ برای جلوگیری از حذف داده همگام‌سازی متوقف شد.');
    const heads = await Promise.all(headFiles.map(f => this.driveDownloadSnapshot(f, opId)));
    this.driveGuard(opId);
    const uniqueHashes = Array.from(new Set(heads.map(h => h.hash)));
    if (heads.length > 1 && uniqueHashes.length === 1) {
      const knownBase = localHash === heads[0].hash || meta.baseHash === heads[0].hash ||
        heads.some(h => meta.baseSnapshotId && h.id === meta.baseSnapshotId);
      const resolved = await this.driveCreateSnapshot(heads[0].dump, heads[0].hash, heads.map(h => h.id),
        'automatic-same-data', opId, { advanceBase: knownBase });
      this.driveGuard(opId);
      if (knownBase) this.driveMetaSave({ baseSnapshotId: resolved.id, baseHash: resolved.hash });
      return this.driveSyncCore(depth + 1, opId);
    }
    if (heads.length === 1) {
      ...
      if (localHash === remote.hash) {
        if (remote.needsMigration) {
          const migrated = await this.driveCreateSnapshot(remote.dump, remote.hash, [remote.id], 'schema-migration', opId);
          this.driveGuard(opId);
          this.driveMetaSave({ baseSnapshotId: migrated.id, baseHash: remote.hash });
          return this.driveSyncCore(depth + 1, opId);
        }
        return this.driveMarkSynced(remote, localHash);
      }
      if (meta.baseHash && localHash === meta.baseHash) {
        const latestLocalHash = await this.driveProgressHash(this.collectBackup());
        this.driveGuard(opId);
        if (latestLocalHash !== localHash) {
          this.driveMetaSave({ dirty: true, dirtyAt: new Date().toISOString(), localHash: latestLocalHash });
          return this.driveSyncCore(depth + 1, opId);
        }
        let after;
        try { after = await this.driveApplySnapshot(remote, opId, localProgressText); }
        catch (err) {
          if (!err || err.code !== 'LOCAL_CHANGED') throw err;
          const newestHash = await this.driveProgressHash(this.collectBackup());
          this.driveGuard(opId);
          this.driveMetaSave({ dirty: true, dirtyAt: new Date().toISOString(), localHash: newestHash });
          return this.driveSyncCore(depth + 1, opId);
        }
        if (after.hash === remote.hash) return this.driveMarkSynced(remote, after.hash);
        const migrated = await this.driveCreateSnapshot(after.dump, after.hash, [remote.id], 'local-migration', opId);
        this.driveGuard(opId);
        this.driveMetaSave({ baseSnapshotId: migrated.id, baseHash: after.hash });
        return this.driveSyncCore(depth + 1, opId);
      }
      if ((meta.baseSnapshotId && meta.baseSnapshotId === remote.id) || (meta.baseHash && meta.baseHash === remote.hash)) {
        const created = await this.driveCreateSnapshot(dump, localHash, [remote.id], 'local-change', opId);
        this.driveGuard(opId);
        this.driveMetaSave({ baseSnapshotId: created.id, baseHash: localHash });
        return this.driveSyncCore(depth + 1, opId);
      }
      if (!meta.baseHash && !this.driveMeaningful(dump)) {
        const latestLocalHash = await this.driveProgressHash(this.collectBackup());
        this.driveGuard(opId);
        if (latestLocalHash !== localHash) {
          this.driveMetaSave({ dirty: true, dirtyAt: new Date().toISOString(), localHash: latestLocalHash });
      ...
      if (!meta.baseHash && !this.driveMeaningful(remote.dump)) {
        const created = await this.driveCreateSnapshot(dump, localHash, [remote.id], 'replace-empty-cloud', opId);
        this.driveGuard(opId);
        this.driveMetaSave({ baseSnapshotId: created.id, baseHash: localHash });
        return this.driveSyncCore(depth + 1, opId);
      }
      const parent = meta.baseSnapshotId && fileById[meta.baseSnapshotId] ? [meta.baseSnapshotId] : [];
      const branch = await this.driveCreateSnapshot(dump, localHash, parent, 'conflicting-local-branch', opId);
      this.driveGuard(opId);
      this.driveMetaSave({ baseSnapshotId: branch.id, baseHash: localHash });
      return this.driveSyncCore(depth + 1, opId);
    }

    const localHead = heads.find(h => h.hash === localHash);
    if (!localHead) {
      if (!meta.baseHash && !this.driveMeaningful(dump)) {
        this.driveGuard(opId);
        this.driveShowConflict(heads, localHash);
        return;
      }
      const parent = meta.baseSnapshotId && fileById[meta.baseSnapshotId] ? [meta.baseSnapshotId] : [];
      const branch = await this.driveCreateSnapshot(dump, localHash, parent, 'conflicting-local-branch', opId);
    ...
    this.driveGuard(opId);
    this.driveMetaSave({ baseSnapshotId: localHead.id, baseHash: localHash, dirty: false });
    this.driveShowConflict(heads, localHash);
  }
  async driveSynchronize() {
    if (this._driveConflict) {
      if (this._mounted) this.setState({ driveConflictOpen: true });
      return;
    }
    if (this._driveSyncing) return;
    if (!navigator.onLine) return this.driveSetStatus('offline', 'اینترنت قطع است؛ پیشرفت روی این دستگاه محفوظ است.');
    if (!this.driveTokenValid()) return this.driveSetStatus('reauth', 'برای بکاپ Drive دوباره روی اتصال کلیک کن.');
    const opId = this.driveBeginOperation();
    this._driveSyncing = opId; clearTimeout(this._driveSyncTimer);
    this.driveArmOperationTimeout(opId);
    this.driveSetStatus('syncing', 'در حال بررسی نسخه‌های Google Drive…');
    try {
      await this.driveLoadAccount(opId);
      this.driveGuard(opId);
      if (await this.driveResumePending(opId)) this.driveSetStatus('syncing', 'ادامهٔ بکاپ ناتمام انجام شد؛ در حال بررسی…');
      this.driveGuard(opId);
      await this.driveSyncCore(0, opId);
    } catch (err) {
      if ((err && (err.code === 'CANCELLED' || err.name === 'AbortError')) || opId !== this._driveOpId) return;
      if (err && err.code === 'REAUTH') this.driveSetStatus('reauth', 'اتصال Drive تمام شد؛ برای ادامه دوباره کلیک کن.');
      else if ((err && err.code === 'NETWORK') || !navigator.onLine) this.driveSetStatus('offline', 'ارتباط قطع شد؛ پیشرفت روی این دستگاه ذخیره شده و بعداً همگام می‌شود.');
      else this.driveSetStatus('error', (err && err.message ? err.message : 'همگام‌سازی انجام نشد.') + ' پیشرفت روی این دستگاه محفوظ است.');
    } finally {
      if (this._driveSyncing === opId) this._driveSyncing = false;
      if (this._driveOpId === opId) { this._driveAbortController = null; this.driveClearOperationTimeout(); }
    }
  }
  async resolveDriveConflict(snapshotId) {
    const conflict = this._driveConflict;
    if (!conflict || this._driveResolving) return;
    const chosen = conflict.choices.find(x => x.id === snapshotId);
    if (!chosen) return;
    if (!this.driveTokenValid()) {
      if (this._mounted) this.setState({ driveConflictOpen: false });
      return this.driveSetStatus('reauth', 'برای ثبت انتخاب، دوباره به Google Drive وصل شو.');
    }
    const opId = this.driveBeginOperation();
    this._driveResolving = opId; this.driveArmOperationTimeout(opId);
    this.driveSetStatus('syncing', 'در حال ثبت انتخاب و حفظ هر دو نسخه…');
    try {
      const localDump = this.collectBackup(), localProgressText = this.driveProgressText(localDump);
      const localHash = await this.driveHash(localProgressText);
      this.driveGuard(opId);
      const files = await this.driveListSnapshots(opId), currentHeads = this.driveHeads(files), ids = currentHeads.map(f => f.id).sort();
      this.driveGuard(opId);
      if (localHash !== conflict.localHash || ids.join('|') !== conflict.headIds.join('|')) {
        this._driveConflict = null;
        if (this._mounted) this.setState({ driveConflictOpen: false });
        this.driveSetStatus('pending', 'در این فاصله نسخه‌ای تغییر کرد؛ فهرست انتخاب دوباره تازه می‌شود.');
        if (this._driveResolving === opId) this._driveResolving = false;
        return this.driveSynchronize();
      }
      const resolved = await this.driveCreateSnapshot(chosen.dump, chosen.hash, ids, 'user-resolution', opId,
        { localHashBefore: localHash });
      this.driveGuard(opId);
      const latestLocalDump = this.collectBackup(), latestProgressText = this.driveProgressText(latestLocalDump);
      const latestLocalHash = await this.driveHash(latestProgressText);
      this.driveGuard(opId);
      if (latestLocalHash !== localHash || latestProgressText !== localProgressText) {
        try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
        this.driveMetaSave({ dirty: true, dirtyAt: new Date().toISOString(), localHash: latestLocalHash });
        this._driveConflict = null;
        if (this._mounted) this.setState({ driveConflictOpen: false });
        this.driveSetStatus('pending', 'هنگام ثبت انتخاب، پیشرفت این دستگاه تغییر کرد؛ نسخه‌ها دوباره بررسی می‌شوند.');
        if (this._driveResolving === opId) this._driveResolving = false;
        return this.driveSynchronize();
      }
      let finalHash = localHash;
      if (localHash !== chosen.hash) finalHash = (await this.driveApplySnapshot(resolved, opId, latestProgressText)).hash;
      this.driveGuard(opId);
      this.driveMetaSave({ baseSnapshotId: resolved.id, baseHash: chosen.hash, localHash: finalHash,
        dirty: finalHash !== chosen.hash });
      try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
      this._driveConflict = null;
      if (this._mounted) this.setState({ driveConflictOpen: false });
      if (finalHash === chosen.hash) this.driveMarkSynced(resolved, chosen.hash);
      else this.driveSetStatus('pending', 'نسخهٔ انتخابی بازیابی شد؛ تغییر سازگاری محلی در صف بکاپ است.');
      if (this._driveResolving === opId) this._driveResolving = false;
      return this.driveSynchronize();
    } catch (err) {
      if ((err && (err.code === 'CANCELLED' || err.name === 'AbortError')) || opId !== this._driveOpId) return;
      if (err && err.code === 'LOCAL_CHANGED') {
        try { localStorage.removeItem(DRIVE_PENDING_KEY); } catch (e) {}
        const newestHash = await this.driveProgressHash(this.collectBackup());
        if (opId !== this._driveOpId) return;
        this.driveMetaSave({ localHash: newestHash, dirty: true, dirtyAt: new Date().toISOString() });
        this._driveConflict = null;
        if (this._mounted) this.setState({ driveConflictOpen: false });
        this.driveSetStatus('pending', 'پیشرفت این دستگاه هنگام ثبت انتخاب تغییر کرد؛ نسخه‌ها دوباره بررسی می‌شوند.');
        if (this._driveResolving === opId) this._driveResolving = false;
        return this.driveSynchronize();
      }
      if (err && err.code === 'REAUTH' && this._mounted) this.setState({ driveConflictOpen: false });
      this.driveSetStatus(err && err.code === 'REAUTH' ? 'reauth' : 'error', (err && err.message ? err.message : 'ثبت انتخاب انجام نشد.') + ' هیچ نسخه‌ای حذف نشد.');
    } finally {
      if (this._driveResolving === opId) this._driveResolving = false;
      if (this._driveOpId === opId) { this._driveAbortController = null; this.driveClearOperationTimeout(); }
    }
  ...
  closeDriveConflict() {
    if (this._mounted) this.setState({ driveConflictOpen: false });
    this.driveSetStatus('conflict', 'تصمیم‌گیری عقب افتاد؛ هر دو نسخه در Drive محفوظ‌اند.');
  }
  driveUiState() {
    const status = (this.state && this.state.driveStatus) || 'checking';
    const map = {
      checking:     ['ph ph-spinner-gap', '#84c5d9', 'در حال بررسی…'],
      unsupported:  ['ph ph-cloud-slash', '#d98f8f', this.driveOnlineUrl() ? 'انتقال به نسخهٔ آنلاین' : 'نیاز به نسخهٔ آنلاین'],
      unconfigured: ['ph ph-wrench', '#e0a458', 'در انتظار راه‌اندازی'],
      disconnected: ['ph ph-google-drive-logo', '#84c5d9', 'اتصال به Google Drive'],
      reauth:       ['ph ph-plugs-connected', '#e0a458', 'اتصال دوباره'],
      connecting:   ['ph ph-spinner-gap', '#84c5d9', 'در حال اتصال…'],
      syncing:      ['ph ph-arrows-clockwise', '#84c5d9', 'در حال همگام‌سازی…'],
      synced:       ['ph-fill ph-cloud-check', '#8fd9c1', 'همگام‌سازی دوباره'],
      pending:      ['ph ph-cloud-arrow-up', '#e0a458', 'همگام‌سازی حالا'],
      offline:      ['ph ph-wifi-slash', '#e0a458', 'تلاش دوباره'],
      conflict:     ['ph ph-warning-diamond', '#e0a458', 'انتخاب نسخه'],
      error:        ['ph ph-warning-circle', '#d98f8f', 'تلاش دوباره']
    };
    const row = map[status] || map.error, color = row[1];
    const standalone = (window.matchMedia && window.matchMedia('(display-mode: standalone)').matches) || window.navigator.standalone === true;
    return { icon: row[0], color: color, button: status === 'connecting' && this._driveAuthPending ? 'لغو تلاش' : row[2],
      busy: status === 'checking' || status === 'syncing' || (status === 'connecting' && !this._driveAuthPending),
      cardStyle: 'padding:14px 15px;border-radius:14px;background:' + color + '0d;border:1px solid ' + color + '42;margin-bottom:16px',
      buttonStyle: 'display:flex;align-items:center;justify-content:center;gap:7px;padding:9px 13px;border-radius:9px;background:' + color + '18;border:1px solid ' + color + '66;color:' + color + ';font-size:12px',
      headerStyle: 'width:44px;height:44px;border-radius:11px;background:' + color + '12;border:1px solid ' + color + '55;color:' + color + ';font-size:16px;display:grid;place-items:center',
      canDisconnect: !!this.driveMetaLoad().enabled, showInstall: this.driveEnvironment() === 'ready' && !standalone };
  }
  driveConflictChoices() {
    const conflict = this._driveConflict;
    if (!conflict) return [];
    return conflict.choices.map((x, i) => ({
      label: x.isLocal ? 'پیشرفت همین دستگاه' : 'پیشرفت ' + (x.deviceLabel || 'دستگاه دیگر'),
      badge: x.isLocal ? 'همین دستگاه' : 'نسخهٔ Drive', date: this.driveFormatTime(x.createdAt),
      summary: this.driveSummaryText(x.summary), icon: x.isLocal ? 'ph ph-device-mobile' : 'ph ph-cloud',
      style: 'width:100%;display:flex;align-items:center;gap:11px;text-align:right;padding:13px;border-radius:11px;background:rgba(233,233,237,.035);border:1px solid ' + (x.isLocal ? 'rgba(132,197,217,.55)' : 'rgba(233,233,237,.14)') + ';color:#e9e9ed',
      choose: () => this.resolveDriveConflict(x.id)
    }));
  }
  ...
  componentWillUnmount() {
    ...
    this.driveClearAuthTimeout();
    this.driveCancelOperations(); this._driveSyncing = false; this._driveResolving = false;
    clearInterval(this.iv); clearInterval(this.csIv); clearInterval(this.dIv); clearInterval(this._driveWatchIv);
    clearTimeout(this._driveSyncTimer); clearTimeout(this._driveTokenTimer);
    if (this._driveOnOnline) window.removeEventListener('online', this._driveOnOnline);
    if (this._driveOnOffline) window.removeEventListener('offline', this._driveOnOffline);
    if (this._driveOnVisible) document.removeEventListener('visibilitychange', this._driveOnVisible);
    if (this._driveOnInstall) window.removeEventListener('beforeinstallprompt', this._driveOnInstall);
    this.lsAuto = false;
  ...
  renderVals() {
    ...
    const driveView = this.driveUiState(), driveChoices = this.driveConflictChoices();
    const ua = (navigator && navigator.userAgent) || '';
    ...
    const vals = Object.assign(this.exVals(), this.courseVals(), this.listenVals(), this.discVals(), this.sentVals(), {
      ...
      driveStatus: s.driveStatus, driveMessage: s.driveMessage, driveIcon: driveView.icon,
      driveCardStyle: driveView.cardStyle, driveButtonStyle: driveView.buttonStyle,
      driveHeaderStyle: driveView.headerStyle, driveButtonLabel: driveView.button,
      driveBusy: driveView.busy, driveLastSync: s.driveLastSync,
      driveHasLastSync: !!s.driveLastSync, driveAccount: s.driveAccount,
      driveHasAccount: !!s.driveAccount, driveCanDisconnect: driveView.canDisconnect,
      driveShowInstall: driveView.showInstall, driveInstallHelpOpen: !!s.installHelpOpen,
      driveInstallHelp: installHelpText, driveInstallLabel: s.installAvailable ? 'نصب برنامه' : 'افزودن به صفحهٔ اصلی',
      driveAction: () => { if (!driveView.busy) this.driveConnect(); },
      driveDisconnect: () => this.driveDisconnect(), installShortcut: () => this.installShortcut(),
      closeInstallHelp: () => this.setState({ installHelpOpen: false }),
      driveConflictOpen: !!s.driveConflictOpen && driveChoices.length > 0,
      driveConflictChoices: driveChoices, closeDriveConflict: () => this.closeDriveConflict(),
      navTabs: NAV.map(t => {
